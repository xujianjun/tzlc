...
class tree
{
	...
	//拓展get_node，add bu xjj
	public function get_nodes($nids, $options = array()){
		$ids = implode(',', $nids);
		$nodes = $this->db->all("
			SELECT
				s.".implode(", s.", $this->options['structure']).",
			...
			WHERE
				s.".$this->options['structure']['id']." = d.".$this->options['data2structure']." AND
				s.".$this->options['structure']['id']." in (".$ids.")
			ORDER BY s.".$this->options['structure']['left']
		);
		if(!$nodes) {
			throw new Exception('Node does not exist');
		}
		...
		foreach ($nodes as $key=>$node){
			if(!$nodeType){
				$nodeType = $node[$this->options['structure']['type']];
				if ($nodeType == 'article'){
					$temOption = $this->options['data'];
					$this->options['data'] = array_merge($this->options['data'], array('summary', 'content'));
					$nodeDetail = $this->get_node($node['id']);
					$this->options['data'] = $temOption;

					$nodes[$key]['summary'] = $nodeDetail['summary'];
					$nodes[$key]['content'] = htmlspecialchars_decode($nodeDetail['content']);
				}
				continue;
			}
			if ($nodeType == 'article' || $node[$this->options['structure']['type']] != $nodeType){
				unset($nodes[$key]);
			}
		}
		foreach ($nodes as $key=>$node){
			if(isset($options['with_children'])) {
				$nodes[$key]['children'] = $this->get_children($node[$this->options['structure']['id']], isset($options['deep_children']), $options['page_children']);
			}
			if(isset($options['with_path'])) {
	...
	public function set_node($id, $params){
		$setStr = '';
		foreach ($params as $key=>$value){
			...
			if ($value){
				...
				$setStr .= $setStr ? ','.$key.'="'.$value.'"' : $key.'="'.$value.'"';
			}
		}
		...
		try {
			...
			$sql = 'update '.$this->options['data_table'].' set '.$setStr.' where '.$this->options['data2structure'].'='.$id;
			$this->db->query($sql);
		} catch(Exception $e) {
			return false;
		}
	...
	public function get_children($id, $recursive = false, $page = 0, $pagesize = 1000) {
		$sql = false;
		...
		if($recursive) {
			...
			$sql = "
				...
				ORDER BY
					s.".$this->options['structure']['left'].$limitSql
			;
		}
		else {
			$sql = "
	...
	public function mk($parent, $position = 0, $data = array(), $type='file') {
		$parent = (int)$parent;
	...
	public function reconstruct($analyze = true) {
		...
		while ($counter <= $maxcounter) {
			if(!$this->db->query("" .
				...
				"ORDER BY temp_tree.".$this->options['structure']["left"]." ASC LIMIT 1"
			)) { return false; }
		...
		// fix positions
		$nodes = $this->db->get("SELECT ".$this->options['structure']['id'].", ".$this->options['structure']['parent_id']." FROM ".$this->options['structure_table']." ORDER BY ".$this->options['structure']['parent_id'].", ".$this->options['structure']['left']);
		$last_parent = false;
		$last_position = false;
	...
	public function dump() {
		...
		foreach($nodes as $node) {
			...
			echo $node[$this->options['structure']["id"]]." ".$node["title"]." (".$node[$this->options['structure']["left"]].",".$node[$this->options['structure']["right"]].",".$node[$this->options['structure']["level"]].",".$node[$this->options['structure']["parent_id"]].",".$node[$this->options['structure']["position"]].")" . "\n";
		}
		echo str_repeat("-",40);
		echo "\n\n";
	}
